...
def main():
    print("𝗦𝗔𝗜𝗙 𝗨𝗦𝗘𝗥 𝗕𝗢𝗧 ")
    print("Hello!! Welcome to Saif User Bot Session Generator\n")
...
def generate_hellbot_session():
    ...
    with PClient(name="helluser", api_id=api_id, api_hash=api_hash, in_memory=True) as hell:
        print("\nYour SAIFUSERBOT SESSION is saved in your telegram saved messages.")
        _session = hell.export_session_string()
        hell_session = hellbot_session(_session)
        hell.send_message(
            "me",
            f"#SAIFUSERBOT_SESSION JOIN @SAIFHELPGC \n\n`{hell_session}`",
        )
...
def generate_insta_session():
    print("Instagram Session For SAIFBOT!")
    cl = IClient()
